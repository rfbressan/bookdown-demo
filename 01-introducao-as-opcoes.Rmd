...
Neste Capítulo[^1] iremos apresentar os instrumentos financeiros conhecidos como opções. Existem dois tipos de opção. Uma opção de compra dá ao detentor o direito de comprar o ativo subjacente até uma determinada data por um determinado preço. Uma opção de venda dá ao titular o direito de vender o ativo subjacente até uma determinada data por um determinado preço.
...
```{r opcao, results = "asis", echo=FALSE}
s0 <- sort(c(seq(9000, 11000, 500), 10200))
premio <- rep(200, 6)
tbl <- tibble(s0, premio) %>% 
  mutate(payoff = pmax(s0 - 10000, 0),
         lucro = payoff - premio, 
         moneyness = c("OTM", "OTM", "ATM", "ITM", "ITM", "ITM"))
kable(tbl,
      col.names = c("Ação", "Prêmio Pago", "Opção no Exercício", "Lucro", "Moneyness"),
      caption = "Exemplo: Comprando call de X com o preço de exercício (strike) de R$ 10.000")
```
...
```{r call, fig.cap = "Perfil de lucro típico de uma compra de call.", echo=FALSE}
ggplot(tbl, aes(x = s0, y = lucro)) +
  geom_line() +
  geom_point(shape = 17, color = "darkgreen", size = 3) +
  labs(x = "Preço da Ação (R$)",
       y = "Lucro da Operação (R$)") +
...
[^1]: Artigo originalmente escrito por Glauber Naue e Erik Kawano, adaptado por Rafael Bressan para este livro.