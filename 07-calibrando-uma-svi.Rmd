...
```{r dados, echo=FALSE, results='asis'}
options(kableExtra.html.bsTable = T)
dados <- read_csv("./R/input/IV_Raw_Delta_surface.csv",
                  col_types = cols(date = col_date(format = "%m/%d/%Y"))) %>% 
  filter(period == 30)
kable(dados,
      digits = 2,
      caption = "Dados reais para exemplo de calibração de uma SVI.",
      booktabs = TRUE) %>% 
...
Ressaltamos aqui que a unidade de volatilidade implícita está em percentuais **ao ano**, equanto que nosso período é de dias corridos. É necessário harmonizar estas medidas de forma que, para volatiliades dadas em percentual ao ano, o período também seja dado em **anos**. Logo nosso $\tau = 30/365$, ou seja, `r format(30/365, digits = 4)`. 
