...
```{r ssvi-cal, results='asis', cache=TRUE}
k <- ssvi_data$k
tau <- ssvi_data$tau
iv <- ssvi_data$iv
...
```{r ssvi-dens, fig.cap="Densidade implícita estimada. Presença de assimetria e leptocurtose a esquerda.", cache=TRUE}
thetadens <- ssvi_data %>% 
  filter(period == 90) %>% 
  pull(theta) %>% 
  `[`(1)
...
```{r dens-area, results = "asis", cache=TRUE}
